...
const WeatherService = {
  ...
  async fetchWeather() {
    try {
      const response = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${CONFIG.CLAVER_LAT}&longitude=${CONFIG.CLAVER_LON}&current_weather=true`
      );
      ...
      if (data.current_weather && typeof data.current_weather.temperature === 'number') {
        return {
          ...
          timestamp: Date.now(),
          location: {
            lat: CONFIG.CLAVER_LAT,
            lon: CONFIG.CLAVER_LON,
            name: 'Claver, Surigao del Norte',
          },
        };
      }
